...
from serviceregistrator.registrator import (
    ServiceRegistrator, ConsulConnectionError, Ports,
    my_default_user_agent, _USER_AGENT,
)
...
def make_registrator(**ctx_overrides):
    ...
    with patch.object(ServiceRegistrator, '_init_consul'), \
         patch.object(ServiceRegistrator, '_init_docker'):
        context = Mock()
        context.options = {
            'debug': False,
            'tags': '',
...
class TestMakeCheck(unittest.TestCase):
    ...
    def test_http_check(self):
        service = Service('cid', 'sid', 'sname', '1.2.3.4', 80,
                          tags=[], attrs={'check_http': '/health', 'check_interval': '10s'})
